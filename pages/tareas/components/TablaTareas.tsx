...
function TablaTareas({ tareasHeaders, tareasData, handleUpdate, handleDelete }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
          {
            tareasHeaders?.map((header) => (
      ...
      <TableBody>
        {
          tareasData?.map((data) => (
            <TableRow key={`row_${data.id}`}>
              <TableHead className="text-white">{data.nombre}</TableHead>
              <TableHead className="text-white">{data.descripcion}</TableHead>
              <TableHead className="text-white">{data.estado}</TableHead>
              <TableHead className="text-white flex flex-row">
                <button 
